	...
	<script>
		...
		function add()
		{
			...
			$('.modal-title').text('Add: Upazila'); // Set Title to Bootstrap modal title
		}
		...
		function edit(id)
		{
			...
			$.ajax({
				...
				success: function(data)
				{
					$('[name="upazila_id"]').val(data.upazila_id);             
					$('[name="upazila_name"]').val(data.upazila_name);                   
					$('[name="upazila_code"]').val(data.upazila_code);                   
					$('[name="district_id"]').val(data.district_id);                   
					$('[name="status"]').val(data.status);                     
					$('#modal_form').modal('show'); // show bootstrap modal when complete loaded
					$('.modal-title').text('Edit: Upazila'); // Set title to Bootstrap modal title
				}
			});
		}
	</script>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<form class="form-horizontal" role="form" method="POST" id="new_form">
					...
					<input type="hidden" class="form-control" value="" name="upazila_id" id="id">

					<div class="form-group">
						<label class="control-label col-md-4">Upazila Name :</label>
						<div class="col-md-6">
							<input type="text" name="upazila_name" id="upazila_name" class="form-control" placeholder="Upazila Name">
							<span class="help-block"></span>
						</div>
					</div>
					
					<div class="form-group">
						<label class="control-label col-md-4">Upazila Code :</label>
						<div class="col-md-6">
							<input type="text" name="upazila_code" id="upazila_code" class="form-control" placeholder="Upazila Code">
							<span class="help-block"></span>
						</div>
					</div>
					<div class="form-group">
