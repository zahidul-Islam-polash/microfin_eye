	...
	<script>
		...
		function add()
		{
			...
			$('.modal-title').text('Add: Samity Config'); // Set Title to Bootstrap modal title
		}
		...
		function edit(id)
		{
			...
			$.ajax({
				...
				success: function(data)
				{
					$('[name="samity_config_id"]').val(data.samity_config_id);             
					$('[name="max_member_per_samity"]').val(data.max_member_per_samity);                   
					$('[name="is_show_loan_product"]').val(data.is_show_loan_product);                
					$('[name="status"]').val(data.status);                     
					$('#modal_form').modal('show'); // show bootstrap modal when complete loaded
					$('.modal-title').text('Edit: Samity Config'); // Set title to Bootstrap modal title
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<form class="form-horizontal" role="form" method="POST" id="new_form">
					...
					<input type="hidden" class="form-control" value="" name="samity_config_id" id="id">
						<div class="form-group row m-b-15" style="padding-left: 30px;">
							<label class="col-form-label col-md-5">Maximum Member per Samity</label>
							<div class="col-md-6">
								<input type="text" class="form-control" value="" name="max_member_per_samity" id="max_member_per_samity">
								<small class="f-s-12 text-grey-darker"></small>
							</div>
						</div>
						<div class="form-group row m-b-15" style="padding-left: 30px;">
							<label class="col-form-label col-md-5">Is product Shown in Samity Page </label>
							<div class="col-md-6">
								<select class="form-control m-b-5" name="is_show_loan_product" id="is_show_loan_product">
