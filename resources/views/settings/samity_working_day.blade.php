	...
	<script>
		...
		function add()
		{
			...
			$('.modal-title').text('Add: Samity Working Day'); // Set Title to Bootstrap modal title
		}
		...
		function edit(id)
		{
			...
			$.ajax({
				...
				success: function(data)
				{
					$('[name="working_day_id"]').val(data.working_day_id);             
					$('[name="working_day_name"]').val(data.working_day_name);                                   
					$('[name="status"]').val(data.status);                     
					$('#modal_form').modal('show'); // show bootstrap modal when complete loaded
					$('.modal-title').text('Edit: Samity Working Day'); // Set title to Bootstrap modal title
				}
			});
		}
	</script>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<form class="form-horizontal" role="form" method="POST" id="new_form">
					...
					<input type="hidden" class="form-control" value="" name="working_day_id" id="id">

					<div class="form-group">
						<label class="control-label col-md-4">Working Day Name :</label>
						<div class="col-md-6">
							<input type="text" name="working_day_name" id="working_day_name" class="form-control" placeholder="Working Day Name">
