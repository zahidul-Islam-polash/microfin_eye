	...
	<script>
		...
		function add()
		{
			...
			$('.modal-title').text('Add: Member Samity Transfer'); // Set Title to Bootstrap modal title
		}
		...
		function edit(id)
		{
			...
			$.ajax({
				...
				success: function(data)
				{
					$('[name="transfer_id"]').val(data.transfer_id);             
					$('[name="member_id"]').val(data.member_id);                                   
					$('[name="current_branch"]').val(data.current_branch);                                   
					$('[name="new_samity"]').val(data.new_samity);                                   
					$('[name="new_member_code"]').val(data.new_member_code);                                   
					$('[name="transfer_date"]').val(data.transfer_date);                                   
					$('[name="status"]').val(data.status);                     
					$('#modal_form').modal('show'); // show bootstrap modal when complete loaded
					$('.modal-title').text('Edit: Member Samity Transfer'); // Set title to Bootstrap modal title
				}
			});
		}
	</script>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<form class="form-horizontal" role="form" method="POST" id="new_form">
					...
					<input type="hidden" class="form-control" value="" name="transfer_id" id="id">
					...
					<div class="form-group">
						...
						<div class="col-md-6">
							<input type="text" name="current_branch" id="current_branch" class="form-control">
							<span class="help-block"></span>
						</div>
					</div>
					<div class="form-group">
						<label class="control-label col-md-4">New Samity :</label>
						<div class="col-md-6">
							<select name="new_samity" id="new_samity" class="form-control">
					...
					<div class="form-group">
						<label class="control-label col-md-4">New Member Code :</label>
						<div class="col-md-6">
							<input type="text" name="new_member_code" id="new_member_code" class="form-control">
							<span class="help-block"></span>
						</div>
					</div>
					<div class="form-group">
